  ...
  async getSattledRange(range) {
    try {
      ...
      const data = await con.query(
        ...
        ' WHERE `transaction`.`day` BETWEEN "2021-01-01" AND "2035-12-30" AND main.`status`=3 AND `transaction`.expenceId IS NULL AND `transaction`.loanType="L" GROUP BY `transaction`.mainId ORDER BY `transaction`.id DESC'
      );
      return data;
    } catch (error) {
